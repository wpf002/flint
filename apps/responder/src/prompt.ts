...
const DESIGN_LINES = [
  ...
  '- Set text and background colors explicitly, on the page and on every control (inputs, buttons, chips),',
  '  with readable contrast in light and dark mode. The phone screenshot is taken in dark mode.',
...
export function systemPrompt(
  ...
  return [
    ...
    '- If the goal is genuinely met, set "done": true and leave "next" null. Answering the part you were',
    '  asked is not the goal being met: if the goal is to build something, it is met when the thing exists',
    '  and a run in this thread shows it working. Otherwise hand off to whoever does the next part.',
