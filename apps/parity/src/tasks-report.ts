...
import { vendorAllowed, type Exclusion, type TaskContext } from './task-privacy.js';
...
/**
 * Every (prompt, vendor) the allowlist kept apart: as a competitor (it wasn't
 * asked) and as a judge (it didn't judge). A prompt Flint failed has no context,
 * so its template's class decides.
 */
export function computeExclusions(opts: {
  ...
  for (const p of opts.prompts) {
    const privacy = opts.contexts.get(p.id)?.privacy ?? p.privacy;
    for (const c of opts.competitors) {
      const vendor = privacyVendorOf(c);
      if (!vendorAllowed(vendor, privacy, opts.personalVendors)) out.push({ promptId: p.id, templateId: p.templateId, vendor, role: 'competitor', privacy });
    }
    for (const vendor of new Set(opts.judgeVendors)) {
      if (!vendorAllowed(vendor, privacy, opts.personalVendors)) out.push({ promptId: p.id, templateId: p.templateId, vendor, role: 'judge', privacy });
    }
  }
  return out;
}
 ...
 * Flint wins, so this is a lower bound for Flint.
 */
export function tasksStrict(opts: {
  ...
  for (const c of opts.competitors) {
    ...
    const flintFailures = opts.flintFailed.filter((p) => vendorAllowed(privacyVendorOf(c.name), p.privacy, opts.personalVendors)).length;
    ...
    out.push({ competitor: c.name, competitorModel: s?.competitorModel ?? c.model, total, flintFailures, n, winRate: winRate(total), p, signal, verdict: verdictOf(total.wins, total.losses, signal) });
  }
  return out;
}
...
export function renderTasksReport(r: TasksReportInput): string {
  ...
  L.push(`Personal prompts (email, calendar, Drive, memory, money) shared with: ${r.personalVendors.join(', ')}.`);
  L.push(
    r.contextChars === undefined
      ...
      : `Each competitor got the request plus the data Flint's tools returned (each result up to ${r.contextChars.toLocaleString('en-US')} characters) and, for memory tasks, the facts Flint recalled. The head-to-head is answer quality on the same data; tool selection is scored separately below.`,
  );
  ...
  if (summaries.length === 0) L.push('_No judgments yet._', '');
    ...
    for (const s of summaries) {
      L.push(`| ${s.competitor} (\`${s.competitorModel}\`) | ${s.total.wins} | ${s.total.losses} | ${s.total.ties} | ${pct(s.winRate)} | ${s.p.toPrecision(3)} | ${s.signal} | ${s.verdict} |`);
    }
  ...
  const strict = tasksStrict({ summaries, competitors, flintFailed, personalVendors: r.personalVendors });
  if (strict.length) {
  ...
  if (summaries.length) {
    ...
    L.push('', 'Each cell: Flint win rate (W-L-T) on that system\'s tasks. Small cells are noise; a "—" means no judged pair (not asked, excluded by the privacy allowlist, or not judged yet).', '');
  }
  ...
  if (sel.n === 0) L.push('_No Flint answers yet._', '');
    ...
    L.push(
      'Tool selection is Flint-only: competitors are handed what Flint retrieved, so a miss here costs both sides the same data in the head-to-head. `calculate` is never required. Extra calls are listed, not failed.',
      '',
    );
  }
  ...
  L.push('## Privacy', '');
  L.push(
    `Default: personal-comms and personal-finance tasks go only to vendors on the allowlist (${r.personalVendors.join(', ')}), as competitor context and as judge input; \`--share-personal-with\` extends it. A task's class is raised by any tool Flint called whose data is more private than the template's class. Recalled memory is kept only for tasks whose point is memory.`,
    '',
  );
  ...
  const withheld = [...r.contexts.values()].filter((c) => c.memoryWithheld > 0).length;
  if (withheld) L.push(`Recalled memory withheld from competitors and judges on ${withheld} task(s) where memory isn't the task (it is personal, and the answer shouldn't need it).`, '');
...
const HEADER =
  'ts,suite,run,task_set,subject,competitor,competitor_model,judge_model,n,flint_wins,competitor_wins,ties,flint_win_rate,p_value,signal,judge_errors,strict_n,strict_win_rate,flint_failures,tool_selection_correct,tool_selection_n,share_personal_with';

/** One row per competitor per invocation that judged something new; the last row per (run, competitor) is the result. */
